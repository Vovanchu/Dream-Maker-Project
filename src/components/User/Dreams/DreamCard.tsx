...
export const DreamCard = ({ dream }: IDreamCardProps) => {
  ...
  const budget = parseFloat(dream.target_budget);
  const collected = Math.floor(budget * 0.72);
  const progress = Math.floor((collected / budget) * 100);
  ...
  return (
    <Card className="overflow-hidden rounded-2xl border border-border bg-card hover:shadow-xl transition-all">
      <div className="relative h-48 w-full overflow-hidden">
        <img
          src={
            dream.image_url && dream.image_url.length > 0
              ? dream.image_url
              : NotFound
          }
      ...
      <div className="p-5 space-y-3">
        ...
        <div className="flex justify-between text-sm text-muted-foreground">
          <span>
            {collected.toLocaleString()} / {budget.toLocaleString()} грн
          </span>
